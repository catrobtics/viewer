...
import type { Story, StoryContext } from '@storybook/react-vite'
...
import { initI18n } from '@catrobotics/studio-base/i18n'
import TimelineInteractionStateProvider from '@catrobotics/studio-base/providers/TimelineInteractionStateProvider'
import ReadySignalContext from '@catrobotics/studio-base/stories/ReadySignalContext'
...
import waitForFonts from '@catrobotics/studio-base/util/waitForFonts'
...
function useCombinedReadySignal(
  ...
  const ready1: { current: boolean } = useRef(false)
  const ready2: { current: boolean } = useRef(false)

  return useMemo(() => {
    function makeSignal(readyRef: { current: boolean }) {
      if (!readySignal) {
        return undefined
      }
      return () => {
