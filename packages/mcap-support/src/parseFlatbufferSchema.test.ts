...
import { BaseType, Schema, Type } from 'flatbuffers_reflection'
...
describe('parseFlatbufferSchema', () => {
  ...
  it('parses root table schema', () => {
    ...
    const reflectionSchemaBuffer: Buffer = fs.readFileSync(
      new URL('./fixtures/reflection.bfbs', import.meta.url),
    )
    const { datatypes, deserialize } = parseFlatbufferSchema(
      'reflection.Schema',
      reflectionSchemaBuffer,
    )
    const deserialized = deserialize(reflectionSchemaBuffer) as {
      objects: Array<{ name: string }>
      file_ident: string
      file_ext: string
      fbs_files: Array<{ filename: string }>
    }
    ...
    expect(deserialized.fbs_files[0]!.filename.slice(-14)).toEqual('reflection.fbs')
  ...
  it('converts uint8 vectors to uint8arrays', () => {
    ...
    const byteVectorSchemaArray = fs.readFileSync(
      new URL('./fixtures/ByteVector.bfbs', import.meta.url),
    )
