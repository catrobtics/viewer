...
import type { FieldT, SchemaT } from 'flatbuffers_reflection'

import type { MessageDefinitionMap } from './types'

import { ByteBuffer } from 'flatbuffers'
import { BaseType, Parser, Schema, Table } from 'flatbuffers_reflection'
...
function typeForSimpleField(type: BaseType): string {
  switch (type) {
    ...
    default:
      throw new Error(`Unsupported FlatBuffer base type: ${type as number}`)
  }
}
...
export function parseFlatbufferSchema(
  ...
  // schema. This preserves the behavior discussed in upstream pull request 6256.
