...
import { MessagePipelineProvider } from '@catrobotics/studio-base/components/MessagePipeline'
import FakePlayer from '@catrobotics/studio-base/components/MessagePipeline/FakePlayer'
...
describe('useMessageReducer', () => {
  ...
  it('clears everything on seek', () => {
    ...
    const restore = vi.fn().mockReturnValue(1)
    const addMessages = vi.fn()
      .mockImplementation((_, messages) => messages.at(-1).message.value)

    let messages: (typeof message1)[] = []
    let activeData: Partial<PlayerStateActiveData> = {}
    const { result, rerender } = renderHook(
      () => PanelAPI.useMessageReducer({ topics: ['/foo'], restore, addMessages }),
      {
        wrapper: ({ children }) => (
  ...
  it('calls addMessages for messages from a newly subscribed topic that emitted previously', async () => {
    const restore = vi.fn()
    const addMessages = vi.fn()

    restore.mockReturnValue(0)
    addMessages.mockImplementation((_, messages) => messages.at(-1).message.value)
    ...
    const { result, rerender } = renderHook(
      ({ topics }: { topics: string[] }) => {
        return PanelAPI.useMessageReducer<number>({ topics, restore, addMessages })
      },
      {
    ...
    // Subscribing does not invoke restore or addMessages
    expect(restore.mock.calls).toEqual([[undefined]])
    expect(addMessages.mock.calls).toEqual([])
    expect(result.current).toEqual(0)
    ...
    // restore call with undefined, then addMessages called with our subscribed message
    expect(restore.mock.calls).toEqual([[undefined], [undefined]])
    expect(addMessages.mock.calls).toEqual([[0, [messageBar]]])
    expect(result.current).toEqual(2)
  ...
  it('restore called when addMessages changes', async () => {
    ...
    const restore = vi.fn().mockReturnValue(1)
    const initialAddMessages = vi.fn()
      .mockImplementation((_, msgs) => msgs[msgs.length - 1].message.value)
