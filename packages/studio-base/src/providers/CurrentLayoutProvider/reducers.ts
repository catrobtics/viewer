...
import type {
  ...
  StartDragPayload,
  SwapPanelPayload,
} from '@catrobotics/studio-base/context/CurrentLayoutContext/actions'
import type { TabPanelConfig } from '@catrobotics/studio-base/types/layouts'
import type { MosaicDropTargetPosition, PlaybackConfig } from '@catrobotics/studio-base/types/panels'
...
} from '@catrobotics/studio-base/util/layout'
...
function changePanelLayout(
  ...
  const panelIds: string[] = getLeaves(layout ?? null).filter(
...
function closePanel(panelsState: LayoutData, { tabId, root, path }: ClosePanelPayload): LayoutData {
  if (tabId != undefined) {
...
function splitPanel(panelsState: LayoutData, { id, tabId, direction, config, root, path }: SplitPanelPayload): LayoutData {
...
function swapPanel(panelsState: LayoutData, { tabId, originalId, type, config, root, path }: MarkOptional<SwapPanelPayload, 'originalId'>): LayoutData {
  const newId = getPanelIdForType(type)
  let newPanelsState = { ...panelsState }
...
function createTabPanelWithSingleTab(panelsState: LayoutData, { idToReplace, layout, idsToRemove }: CreateTabPanelPayload): LayoutData {
...
function createTabPanelWithMultipleTabs(panelsState: LayoutData, { idToReplace, layout, idsToRemove }: CreateTabPanelPayload): LayoutData {
  const { configById: savedProps } = panelsState
  const newId = getPanelIdForType(TAB_PANEL_TYPE)
  const newLayout = replaceAndRemovePanels({ originalId: idToReplace, newId, idsToRemove }, layout)
...
function moveTab(panelsState: LayoutData, { source, target }: MoveTabPayload): LayoutData {
...
function addPanel(panelsState: LayoutData, { tabId, id, config }: AddPanelPayload) {
  let newPanelsState = { ...panelsState }
...
function dropPanel(panelsState: LayoutData, { newPanelType, destinationPath = [], position, tabId, config }: DropPanelPayload) {
...
function dragWithinSameTab(panelsState: LayoutData, {
  originalLayout,
  sourceTabId,
  position,
  destinationPath,
...
function dragToMainFromTab(panelsState: LayoutData, {
  originalLayout,
  sourceTabId,
  position,
  destinationPath,
...
function dragToTabFromMain(panelsState: LayoutData, {
  originalLayout,
  panelId,
  targetTabId,
  position,
...
function dragToTabFromTab(panelsState: LayoutData, {
  originalLayout,
  panelId,
  sourceTabId,
  targetTabId,
...
function startDrag(panelsState: LayoutData, { path, sourceTabId }: StartDragPayload): LayoutData {
...
function endDrag(panelsState: LayoutData, dragPayload: EndDragPayload): LayoutData {
  const {
    originalLayout,
    originalSavedProps,
    panelId,
