...
import type { DataSourceDialogItem } from '@catrobotics/studio-base/components/DataSourceDialog'
import type {
  LayoutData,
...
import { useGuaranteedContext, useMountedState } from '@catrobotics/hooks'
...
import useCallbackWithToast from '@catrobotics/studio-base/hooks/useCallbackWithToast'
import { AppEvent } from '@catrobotics/studio-base/services/IAnalytics'
import { downloadTextFile } from '@catrobotics/studio-base/util/download'
...
function setterValue<T>(action: SetStateAction<T>, value: T): T {
  if (typeof action === 'function') {
    const updater = action as (previousValue: T) => T
...
export function useWorkspaceActions(): WorkspaceActions {
  ...
  const exportLayoutToFile = useCallback(() => {
    ...
    const name = getCurrentLayoutState().selectedLayout?.name ?? 'catrobotics-layout'
