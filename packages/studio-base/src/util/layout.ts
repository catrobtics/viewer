...
import type { TabConfig, TabLocation, TabPanelConfig } from '@catrobotics/studio-base/types/layouts'
import type {
...
import type { MosaicDirection, MosaicKey } from 'react-mosaic-component/lib/types'
import { filterMap } from '@catrobotics/den/collection'
import Logger from '@catrobotics/log'
import { reportError } from '@catrobotics/studio-base/reportError'
...
export const PANEL_TITLE_CONFIG_KEY = 'catroboticsPanelTitle'
...
export function getParentTabPanelByPanelId(savedProps: SavedProps): {
  [key: string]: string
} {
  return Object.entries(savedProps).reduce(
    (
...
function replaceMaybeTabLayoutWithNewPanelIds(panelIdMap: PanelIdMap) {
...
export function getSaveConfigsPayloadForAddedPanel({
  id,
  config,
  savedProps,
}: {
...
export function validateTabPanelConfig(config?: PanelConfig): config is TabPanelConfig {
...
export function updateTabPanelLayout(layout: MosaicNode<string> | undefined, tabPanelConfig: TabPanelConfig): TabPanelConfig {
...
export function removePanelFromTabPanel(path: MosaicPath = [], config: TabPanelConfig, tabId: string): SaveConfigsPayload {
...
export function createAddUpdates(tree: MosaicNode<string> | undefined, panelId: string, newPath: MosaicPath, position: MosaicDropTargetPosition): MosaicUpdate<string>[] {
  if (tree == undefined) {
    return []
  }
...
export function addPanelToTab(insertedPanelId: string, destinationPath: MosaicPath | undefined, destinationPosition: MosaicDropTargetPosition | undefined, tabConfig: PanelConfig | undefined, tabId: string): SaveConfigsPayload {
...
export function reorderTabWithinTabPanel({
  source,
  target,
  savedProps,
}: {
...
export function moveTabBetweenTabPanels({
  source,
  target,
  savedProps,
}: {
...
export function replaceAndRemovePanels(panelArgs: {
  originalId?: string
  newId?: string
