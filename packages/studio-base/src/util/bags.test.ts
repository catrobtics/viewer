...
import permutations from '@catrobotics/studio-base/test/permutations'
import { fromSec } from '@foxglove/rostime'

import { getBagChunksOverlapCount } from './bags'

function expectChunkOverlaps(elements: { startTime: Time, endTime: Time }[], expectedSize: number) {
  for (const permutation of permutations(elements)) {
    expect(getBagChunksOverlapCount(permutation)).toBe(expectedSize)
  }
}

function times(input: [number, number][]) {
