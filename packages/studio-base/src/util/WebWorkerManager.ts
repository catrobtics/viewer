...
import { setupMainThreadRpc } from '@catrobotics/studio-base/util/RpcMainThreadUtils'
...
interface WorkerListenerState<W> { rpc: Rpc, worker: W, listenerIds: string[] }
...
export default class WebWorkerManager<W extends Channel> {
  ...
  public constructor(createWorker: () => W, maxWorkerCount: number) {
    ...
    this.#workerStates = Array.from({ length: maxWorkerCount })
  ...
  public testing_getWorkerState(id: string): WorkerListenerState<W> | undefined {
    return this.#workerStates.find(
      workerState => workerState?.listenerIds.includes(id) ?? false,
    )
  }
