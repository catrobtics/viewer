...
import { fromDate, toDate } from '@foxglove/rostime'
...
const todDateTimeRegex = /^\d+-\d+-\d+\s+\d+:\d+:\d(?:\d?[^\d\n\r\u2028\u2029]\d+|\d{2,}(?:[^\d\n\r\u2028\u2029]\d+)?)\s[PA]M\s[A-Z$]+/i
export function getValidatedTimeAndMethodFromString({
  text,
  timezone,
}: {
  text?: string
  ...
  const isInvalidRawTime = Number.isNaN(+text)
