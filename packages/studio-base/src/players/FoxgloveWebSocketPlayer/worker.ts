...
function sendWithTransfer(message: FromWorkerMessage, transfer: Transferable[]): void {
    ...
    case 'open':
      try {
        ...
        ws.onerror = (wsEvent) => {
          send({
            type: 'error',
            error: wsEvent instanceof ErrorEvent ? wsEvent.error : wsEvent,
          })
        }
        ws.onopen = (_event) => {
          send({
