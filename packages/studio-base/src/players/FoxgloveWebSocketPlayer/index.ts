...
import type { ParsedChannel } from '@catrobotics/mcap-support'
import type { ParameterValue } from '@catrobotics/studio'
import type { Asset } from '@catrobotics/studio-base/components/PanelExtensionAdapter'
...
import type { MessageWriter } from './MessageWriter'
import { debouncePromise } from '@catrobotics/den/async'
import Log from '@catrobotics/log'
import { parseChannel } from '@catrobotics/mcap-support'
import { estimateObjectSize } from '@catrobotics/studio-base/players/messageMemoryEstimation'
import PlayerProblemManager from '@catrobotics/studio-base/players/PlayerProblemManager'
import {
  PlayerCapabilities,
  PlayerPresence,
} from '@catrobotics/studio-base/players/types'
import rosDatatypesToMessageDefinition from '@catrobotics/studio-base/util/rosDatatypesToMessageDefinition'
import { isMsgDefEqual } from '@foxglove/message-definition'
import { MessageWriter as Ros2MessageWriter } from '@foxglove/rosmsg2-serialization'
import CommonRosTypes from '@foxglove/rosmsg-msgs-common'
import { MessageWriter as Ros1MessageWriter } from '@foxglove/rosmsg-serialization'
import { fromMillis, fromNanoSec, isGreaterThan, isLessThan } from '@foxglove/rostime'
import {
  BinaryOpcode,
  FetchAssetStatus,
  FoxgloveClient,
...
import BrowserSocketAdapter from './BrowserSocketAdapter'
...
export default class FoxgloveWebSocketPlayer implements Player {
  ...
  #open = (): void => {
    ...
    this.#client = new FoxgloveClient({
      ws:
        typeof Worker !== 'undefined'
          ...
          : new BrowserSocketAdapter(this.#url, [FoxgloveClient.SUPPORTED_SUBPROTOCOL]),
    })
    ...
    this.#client.on('error', (err) => {
      ...
      if (err instanceof Error && err.message.includes('insecure WebSocket connection')) {
    ...
    if (clientChannel.encoding === 'json') {
      ...
      const replacer = (_key: string, value: unknown) => {
        ...
        if (typeof length !== 'number') {
          throw new TypeError('Typed array length must be a number')
        }
        return Array.from({ length }, (_, index) => Reflect.get(value, index))
      }
      const message = textEncoder.encode(JSON.stringify(msg, replacer) ?? '')
  ...
  #advertiseChannel(publication: AdvertiseOptions) {
    ...
    let messageWriter: Publication['messageWriter']
    if (ROS_ENCODINGS.includes(encoding)) {
      ...
      try {
        ...
        if (!(datatypes instanceof Map)) {
          throw new TypeError('Datatypes option must be a map')
        }
  ...
  #updateDataTypes(datatypes: MessageDefinitionMap): void {
    let updatedDatatypes: MessageDefinitionMap | undefined
