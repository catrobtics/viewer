...
import { loadDecompressHandlers } from '@catrobotics/mcap-support'
...
export class McapIterableSource implements IIterableSource {
  ...
  public async initialize(): Promise<Initalization> {
    ...
    switch (source.type) {
      case 'file': {
        // Preflight the file so permission failures retain their specific error instead of being
        // converted to a generic "network error" by `file.stream().getReader()`.
