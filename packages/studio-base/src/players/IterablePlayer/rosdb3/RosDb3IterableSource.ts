...
import { basicDatatypes } from '@catrobotics/studio-base/util/basicDatatypes'
import { ROS2_TO_DEFINITIONS, Rosbag2, SqliteSqljs } from '@foxglove/rosbag2-web'
import { stringify } from '@foxglove/rosmsg'
import { add as addTime } from '@foxglove/rostime'
    ...
    for (const topicDef of topicDefs) {
      ...
      if (parsedMsgdef == undefined) {
        problems.push({
          ...
          tip: 'ROS 2 .db3 files do not contain message definitions, so only well-known ROS types are supported. To include message definitions, convert the db3 file to mcap using the mcap CLI. For more information, see: https://catrotics.com',
        })
        continue
      }
