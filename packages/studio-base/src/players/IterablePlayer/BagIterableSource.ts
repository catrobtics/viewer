...
import { getBagChunksOverlapCount } from '@catrobotics/studio-base/util/bags'
import BrowserHttpReader from '@catrobotics/studio-base/util/BrowserHttpReader'
import CachedFilelike from '@catrobotics/studio-base/util/CachedFilelike'
...
export class BagIterableSource implements IIterableSource {
  ...
  public async initialize(): Promise<Initalization> {
    ...
    this.#bag = new Bag(fileLike, {
      ...
      decompress: {
        bz2: (buffer: Uint8Array, size: number) => {
          return Bunzip.decode(buffer, new Uint8Array(size))
        },
    ...
    const numMessagesByConnectionIndex = Array.from({ length: this.#bag.connections.size }).fill(
      0,
    ) as number[]
    this.#bag.chunkInfos.forEach((info) => {
      info.connections.forEach(({ conn, count }) => {
        numMessagesByConnectionIndex[conn] = numMessagesByConnectionIndex[conn]! + count
      })
    })
