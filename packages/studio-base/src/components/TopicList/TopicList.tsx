...
import type { TopicListItem } from './useTopicListSearch'
import { filterMap } from '@catrobotics/den/collection'
import { useLatest } from '@catrobotics/hooks'
import { quoteTopicNameIfNeeded } from '@catrobotics/message-path'
...
import { ContextMenu } from '@catrobotics/studio-base/components/TopicList/ContextMenu'
...
import { MessagePathSelectionProvider } from '@catrobotics/studio-base/services/messagePathDragging/MessagePathSelectionProvider'
...
const useStyles = makeStyles({ name: 'TopicList' })(theme => ({
  root: {
    width: '100%',
    height: '100%',
    overflow: 'hidden',
...
export function TopicList(): React.JSX.Element {
  ...
  const renderRow = useCallback(
    ...
    }: RowComponentProps<typeof itemData>) => {
  ...
  ) satisfies (props: RowComponentProps<typeof itemData>) => React.JSX.Element
            ...
            <AutoSizer>
              {({ width, height }) => (
                <VirtualList
                  ...
                  rowHeight={index => (treeItems[index]?.type === 'topic' ? 50 : 28)}
                  rowProps={itemData}
