...
import type { PanelConfig, SaveConfig } from '@catrobotics/studio-base/types/panels'
import type {
  CSSProperties,
} from 'react'
import type { BuiltinPanelExtensionContext } from './types'
import { useLatest, useSynchronousMountedState, useValueChangedDebugLog } from '@catrobotics/hooks'
...
import { useAppConfiguration } from '@catrobotics/studio-base/context/AppConfigurationContext'
import {
  useExtensionCatalog,
} from '@catrobotics/studio-base/context/ExtensionCatalogContext'
import {
  ...
  useHoverValue,
  useSetHoverValue,
} from '@catrobotics/studio-base/context/TimelineInteractionStateContext'
import useGlobalVariables from '@catrobotics/studio-base/hooks/useGlobalVariables'
import {
  PlayerCapabilities,
} from '@catrobotics/studio-base/players/types'
import {
...
} from '@catrobotics/studio-base/providers/PanelStateContextProvider'
...
export const VERSION_CONFIG_KEY = 'catroboticsConfigVersion'
...
interface PanelExtensionAdapterProps {
   ...
   * numbered catroboticsConfigVersion property in their config to control this.
   */
...
type PanelOnRender = NonNullable<PanelExtensionContext['onRender']>
type RenderFn = (
  this: void,
  ...args: Parameters<PanelOnRender>
) => ReturnType<PanelOnRender>
/**
...
function PanelExtensionAdapter(
  ...
  const partialExtensionContext = useMemo<PartialPanelExtensionContext>(() => {
    const layout: PanelExtensionContext['layout'] = {
      addPanel: (options) => {
        ...
        if (options.position !== 'sibling') {
          throw new Error(`Unsupported panel position: ${String(options.position)}`)
        }
        openSiblingPanel({
          panelType: options.type,
          updateIfExists: options.updateIfExists,
          siblingConfigCreator: existingConfig =>
            options.getState(existingConfig) as PanelConfig,
        })
      },
    }
    ...
    return {
      ...
      subscribe: (topics: ReadonlyArray<Subscription>) => {
        if (!isMounted()) {
          return
        }
  ...
  useLayoutEffect(() => {
    ...
    const onUnmount = initPanel({
      ...
      // eslint-disable-next-line accessor-pairs -- The extension API exposes onRender as a write-only callback registration.
  ...
  const panelChildren = Children.toArray([
    <PanelToolbar key="toolbar" />,
    configTooNew ? <PanelConfigVersionError key="config-version-error" /> : undefined,
    props.children,
    <div key="panel-container" style={{ flex: 1, overflow: 'hidden' }} ref={panelContainerRef} />,
  ])
