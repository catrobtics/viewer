...
import type { Topic as PlayerTopic } from '@catrobotics/studio-base/players/types'

import type { ExtensionNamespace } from '@catrobotics/studio-base/types/Extensions'
...
export interface TopicSchemaConversions {
...
export function forEachSortedArrays<Item>(
  ...
  const cursors = Array.from({ length: arrays.length }).fill(0) as number[]
  if (arrays.length === 0) {
    return
  }
  for (;;) {
    ...
    if (minItem != undefined) {
      forEach(minItem)
      cursors[minCursorIndex] = cursors[minCursorIndex]! + 1
    }
    else {
      break
    }
