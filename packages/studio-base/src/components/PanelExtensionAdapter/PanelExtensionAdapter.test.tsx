...
import type { Immutable, MessageEvent, PanelExtensionContext, RenderState } from '@catrobotics/studio'

import type { AdvertiseOptions } from '@catrobotics/studio-base/players/types'
import type { Fixture } from '@catrobotics/studio-base/stories/PanelSetup'
import type { Time } from '@foxglove/rostime'
import { Condvar, signal } from '@catrobotics/den/async'
import MockPanelContextProvider from '@catrobotics/studio-base/components/MockPanelContextProvider'
...
describe('panelExtensionAdapter', () => {
  ...
  it('should unadvertise when unmounting', async () => {
    expect.assertions(5)
    let count = 0
    let resolveUnadvertised!: () => void
    const unadvertised = new Promise<void>((resolve) => {
      resolveUnadvertised = resolve
    })
  ...
  it('ignores subscriptions after panel unmount', async () => {
    ...
    expect(mockSetSubscriptions.mock.calls).toEqual([
      [expect.any(String), [{ preloadType: 'partial', topic: 'x' }]],
    ])
    unmount()
    expect(mockSetSubscriptions.mock.calls).toEqual([
      [expect.any(String), [{ preloadType: 'partial', topic: 'x' }]],
