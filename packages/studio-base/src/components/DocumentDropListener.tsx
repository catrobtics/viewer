...
import DropOverlay from '@catrobotics/studio-base/components/DropOverlay'
  ...
  const onDrop = useCallback(
    async (ev: DragEvent) => {
      ...
      const filteredFiles = allFiles.filter(file =>
        allowedExtensions.includes(fileExtension(file.name)),
      )
      const filteredHandles = handles?.filter(handle =>
        allowedExtensions.includes(fileExtension(handle.name)),
      )
