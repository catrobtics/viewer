...
import { ExperimentalFeatureSettings } from '@catrobotics/studio-base/components/ExperimentalFeatureSettings'
...
import OsContextSingleton from '@catrobotics/studio-base/OsContextSingleton'
...
const useStyles = makeStyles({ name: 'AppSettingsDialog' })(theme => ({
  layoutGrid: {
    display: 'grid',
    gap: theme.spacing(2),
    height: '70vh',
...
function createAboutItems(websiteUrl: string) {
  return new Map<
    SectionKey,
    {
      subheader: string
  ...
  >([
    [
      'resources',
      {
        ...
        links: [
          ...(isDesktopApp() ? [] : [{ title: 'Desktop app', url: websiteUrl }]),
          { title: 'Browse docs', url: websiteUrl },
          { title: 'Join our community', url: websiteUrl },
        ],
      },
    ],
    [
    ...
    [
      'contact',
      {
        subheader: 'Contact',
        links: [
          { title: 'Give feedback', url: websiteUrl },
          { title: 'Schedule a demo', url: websiteUrl },
        ],
      },
    ],
    [
      'legal',
      {
        subheader: 'Legal',
        links: [
          { title: 'License terms', url: websiteUrl },
          { title: 'Privacy policy', url: websiteUrl },
        ],
      },
    ],
  ])
...
function selectWorkspaceInitialActiveTab(store: WorkspaceContextStore) {
  return store.dialogs.preferences.initialTab
}
...
export function AppSettingsDialog(
  ...
  const websiteUrl = branding.websiteUrl ?? defaultBranding.websiteUrl ?? 'https://catrotics.com'
  const aboutItems = createAboutItems(websiteUrl)
        ...
        <Stack direction="row" fullHeight overflowY="auto">
            ...
            <Stack gap={2}>
              ...
              <Stack>
                ...
                <FormControlLabel
                  ...
                  label={t('debugModeDescription', { productName })}
                />
              </Stack>
            </Stack>
          </section>
          ...
          >
            <Stack gap={2} alignItems="flex-start">
              <header>
                <BrandMark variant="wordmark" className={classes.logo} />
              </header>
              <Stack direction="row" alignItems="center" gap={1}>
                ...
                <CopyButton
                  size="small"
                  getText={() => CATROBOTICS_VERSION?.toString() ?? ''}
                />
              </Stack>
              {[
                aboutItems.get('resources'),
                {
                  ...
                  links: [
                    { title: productName, url: websiteUrl },
                    {
                      title: `${productName} Data Platform`,
                      url: websiteUrl,
                    },
                  ],
                },
