...
import type { ObjectData, TypedData } from './types'

import * as R from 'ramda'

export interface Point { index: number, x: number, y: number, label?: string | undefined }
 ...
 *
 * The generic constraint preserves the relationship between each array property
 * and its element type without erasing values to `any`.
 */

type PointArray = readonly unknown[] | Float32Array

type PointArrayElement<Value> = Value extends Float32Array
  ? number
  : Value extends readonly (infer Element)[]
    ? Element
    : never

export type ExtractPoint<T extends object> = {
  [P in keyof T]-?: PointArrayElement<NonNullable<T[P]>>
} & {
  index: number
...
export function* iterateTyped<T extends object>(
  dataset: T[],
  ...
  for (const slice of dataset) {
    ...
    const first = R.head(Object.values(slice).filter(isPointArray))
    if (first == undefined) {
      continue
    }

    for (let j = 0; j < first.length; j++) {
      for (const key of Object.keys(slice) as (keyof T)[]) {
        const value = slice[key]
        if (!isPointArray(value)) {
          continue
        }
        point[key] = value[j] as never
      }
...
function isPointArray(value: unknown): value is PointArray {
