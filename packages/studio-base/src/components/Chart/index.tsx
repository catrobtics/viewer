...
import type {
  ChartUpdateMessage,
} from '@catrobotics/studio-base/components/Chart/worker/ChartJsMux'
import type { ZoomPluginOptions } from '@foxglove/chartjs-plugin-zoom/types/options'
import type { ChartOptions } from 'chart.js'
import type { ChartData, RpcElement, RpcScales, TypedChartData } from './types'
import { useMountedState } from '@catrobotics/hooks'
import Logger from '@catrobotics/log'
import { scalesEqual } from '@catrobotics/studio-base/components/Chart/scales'
import ChartJsMux from '@catrobotics/studio-base/components/Chart/worker/ChartJsMux'

import Rpc, { createLinkedChannels } from '@catrobotics/studio-base/util/Rpc'
import WebWorkerManager from '@catrobotics/studio-base/util/WebWorkerManager'
...
function Chart(props: Props): React.JSX.Element {
  ...
  const sendWrapperRef = useRef<RpcSend | undefined>(undefined)
  const rpcSendRef = useRef<RpcSend | undefined>(undefined)
  ...
  useLayoutEffect(() => {
    ...
    if (supportsOffscreenCanvas) {
      ...
      void new ChartJsMux(new Rpc(remote))
      rpc = new Rpc(local)
    }
  ...
  const maybeUpdateScales = useCallback(
    (newScales: RpcScales, opt?: { userInteraction: boolean }) => {
      ...
      if (!scalesEqual(oldScales, newScales)) {
  ...
  const getNewUpdateMessage = useCallback(() => {
    ...
    // OffscreenCanvas cannot be initialized with a drawable surface until layout has assigned
    // non-zero dimensions. Defer initialization instead of creating a permanently blank chart.
    if (width === 0 || height === 0) {
      return undefined
    }
  ...
  const updateChart = useCallback(
    async (update: PartialUpdate) => {
      if (initialized.current) {
        ...
        await flushUpdates(rpcSendRef.current ?? undefined)
        return
      }
