...
import type { TypedChartData } from '../types'
import Logger from '@catrobotics/log'

import { maybeCast } from '@catrobotics/studio-base/util/maybeCast'
...
const lastX: InteractionModeFunction = (chart, event, _options, useFinalPosition) => {
  ...
  Interaction.evaluateInteractionItems(chart, 'x', position, (element, datasetIndex, index) => {
    ...
    if (center.x != null && center.x <= position.x) {
...
export default class ChartJSManager {
  ...
  public getElementsAtEvent({ event }: { event: MouseEvent }): RpcElement[] {
    ...
    for (const element of elements) {
      ...
      if (data == undefined || typeof data === 'number' || data.x == null || data.y == null) {
        continue
      }
  ...
  #addFunctionsToConfig(config: ChartOptions): typeof config {
    ...
    if (datalabelsOptions) {
      ...
      const getStaticColor = (context: DatalabelContext): string | CanvasGradient | CanvasPattern => {
        if (typeof staticColor === 'function') {
          return staticColor(context)
        }
        if (Array.isArray(staticColor)) {
          return staticColor[context.dataIndex] ?? 'white'
        }
        return staticColor
      }
      datalabelsOptions.color = (context: DatalabelContext) => {
        ...
        if (value == undefined || typeof value !== 'object') {
          return getStaticColor(context)
        }
        const labelColor = Reflect.get(value, 'labelColor')
