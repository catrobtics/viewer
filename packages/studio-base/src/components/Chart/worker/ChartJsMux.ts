...
import type Rpc from '@catrobotics/studio-base/util/Rpc'
import type {
  ChartData,
  ChartOptions,
} from 'chart.js'
...
import type { InitOpts } from './ChartJSManager'
import PlexMono from '@catrobotics/studio-base/styles/assets/PlexMono.woff2'
import { setupWorker } from '@catrobotics/studio-base/util/RpcWorkerUtils'
...
interface RpcEvent<EventType> { id: string, event: EventType }
...
async function loadDefaultFont(): Promise<FontFace> {
  ...
  if (typeof WorkerGlobalScope !== 'undefined' && globalThis instanceof WorkerGlobalScope) {
    const fonts = Reflect.get(globalThis, 'fonts')
    const add = fonts != undefined && typeof fonts === 'object' ? Reflect.get(fonts, 'add') : undefined
    if (typeof add === 'function') {
      Reflect.apply(add, fonts, [fontFace])
    }
  }
  else {
