...
import type { RpcElement, RpcScales } from '@catrobotics/studio-base/components/Chart/types'
import type {
...
import type { ChartOptions, InteractionMode, ScaleOptions } from 'chart.js'
...
import type { ObjectDataProvider, TypedDataProvider } from './types'
...
import ChartComponent from '@catrobotics/studio-base/components/Chart/index'
import KeyListener from '@catrobotics/studio-base/components/KeyListener'
import { useMessagePipeline } from '@catrobotics/studio-base/components/MessagePipeline'
...
import { Button, buttonClasses, Fade, Tooltip } from '@mui/material'
...
import useProvider, { getBounds, getTypedBounds, mergeNormal, mergeTyped } from './useProvider'
import { VerticalBarWrapper } from './VerticalBarWrapper'

const log = Logger.getLogger(import.meta.url)

const useStyles = makeStyles({ name: 'TimeBasedChart' })(theme => ({
  root: {
    position: 'relative',
  },
  resetZoomButton: {
...
export default function TimeBasedChart(props: Props): React.JSX.Element {
  ...
  const resumeFrame = useRef<(() => void | undefined) | null>(null)
  const requestedResumeFrame = useRef<(() => void | undefined) | null>(null)
  ...
  const onFinishRender = useCallback(() => {
    ...
    if (current) {
      ...
      requestID.current = requestAnimationFrame(() => {
        current()
        requestedResumeFrame.current = null
      })
    }
  }, [])
  ...
  const onMouseMove = useCallback(
    (event: MouseEvent<HTMLDivElement>) => {
      ...
      if (!xInBounds || Number.isNaN(xVal)) {
        setActiveTooltip(undefined)
        clearGlobalHoverTime()
        return
      }
