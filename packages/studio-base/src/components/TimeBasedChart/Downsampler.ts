...
import type { RpcScales } from '@catrobotics/studio-base/components/Chart/types'
import type { ChartDatasets, PlotViewport } from './types'
import { iterateObjects } from '@catrobotics/studio-base/components/Chart/datasets'
...
export class Downsampler {
  ...
  public downsample(): ChartDatasets | undefined {
    ...
    let view: PlotViewport | undefined
    ...
    return this.#datasets.map((dataset) => {
      ...
      const undefinedToNanData = resolved.map((item) => {
        if (item == undefined || Number.isNaN(item.x) || Number.isNaN(item.y)) {
