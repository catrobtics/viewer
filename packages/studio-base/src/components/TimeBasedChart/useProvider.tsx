...
import type { ObjectData, TypedData } from '@catrobotics/studio-base/components/Chart/types'
import type { ChartData, ChartDataset } from 'chart.js'
import type { Bounds, PlotDataProvider, PlotViewport, ProviderState } from './types'

import { iterateObjects, iterateTyped } from '@catrobotics/studio-base/components/Chart/datasets'
...
function makeMerge<T,>(mergeData: (dataA: T, dataB: T) => T) {
  return (a: State<T>, b: State<T>): State<T> => {
    return {
...
export default function useProvider<T>(
  ...
  const [state, setState] = useState<DataState<T> | undefined>()

  const setFull = useCallback((newFull: ProviderState<T>) => {
    setState({
      full: newFull,
      partial: undefined,
    })
