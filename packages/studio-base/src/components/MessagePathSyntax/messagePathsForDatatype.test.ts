...
import { unwrap } from '@catrobotics/den/monads'
...
describe('traverseStructure', () => {
  it('returns whether the path is valid for the structure, plus some metadata', () => {
    ...
    const somePose = unwrap(structure?.nextByName.some_pose)
    if (somePose.structureType !== 'message') {
      throw new Error('Expected some_pose to be a message')
    }
    const dummyArray = unwrap(somePose.nextByName.dummy_array)
    if (dummyArray.structureType !== 'array') {
      throw new Error('Expected dummy_array to be an array')
    }
    ...
    expect(
      ...
      structureItem: somePose.nextByName.x,
    })
    expect(
      traverseStructure(structure, [
        { type: 'name', name: 'some_pose', repr: 'some_pose' },
