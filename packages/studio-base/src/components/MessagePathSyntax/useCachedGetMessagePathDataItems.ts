...
import type {
  GlobalVariables,
} from '@catrobotics/studio-base/hooks/useGlobalVariables'
import type { MessageEvent, Topic } from '@catrobotics/studio-base/players/types'
import type { RosDatatypes } from '@catrobotics/studio-base/types/RosDatatypes'
import { filterMap } from '@catrobotics/den/collection'
import { useDeepMemo, useShallowMemo } from '@catrobotics/hooks'
import {
  parseMessagePath,
...
} from '@catrobotics/studio-base/util/enums'
...
export interface MessagePathDataItem {
...
function getIndexableLength(value: unknown): number | undefined {
...
export function getMessagePathDataItems(
  ...
  function traverse(
    ...
    if (!pathItem) {
      ...
      if (isRecord(value)) {
        traverse(Reflect.get(value, pathItem.name), pathIndex + 1, `${path}.${pathItem.repr}`, next)
      }
    }
    else if (
      pathItem.type === 'slice'
      ...
      const arrayLength = getIndexableLength(value)
      if (arrayLength == undefined) {
        return
      }
      for (let i = startIdx; i <= Math.min(endIdx, arrayLength - 1); i++) {
        const index = i >= 0 ? i : arrayLength + i
        const arrayElement = Reflect.get(new Object(value), index)
        ...
        if (nextPathItem && nextPathItem.type === 'filter') {
          ...
          if (name != undefined) {
            newPath = `${path}[:]{${name}==${Reflect.get(arrayElement, name)}}`
          }
          else {
...
export interface MessageDataItemsByPath {
