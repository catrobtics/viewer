...
import type { DataSourceDialogItem } from '@catrobotics/studio-base/components/DataSourceDialog/DataSourceDialog'
...
import TextMiddleTruncate from '@catrobotics/studio-base/components/TextMiddleTruncate'
...
const useStyles = makeStyles({ name: 'Start' })(theme => ({
  logo: {
    width: 420,
    maxWidth: '100%',
    height: 'auto',
...
function SidebarItems(props: {
  ...
  const { freeUser, teamOrEnterpriseUser } = useMemo(() => {
    const demoItem = {
      id: 'new',
      title: t('newToProduct', { productName }),
      text: t('newToProductDescription'),
      actions: (
        <>
          <Button
            onClick={() => {
    ...
    return {
      ...
      teamOrEnterpriseUser: [
        ...
        },
        {
          id: 'need-help',
          title: t('needHelp'),
          text: t('needHelpDescription', { productName }),
          actions: (
            <>
              <Button
                href="https://catrotics.com"
  ...
  }, [analytics, classes.button, currentUserType, onSelectView, productName, t])
    ...
    switch (currentUserType) {
      case 'unauthenticated':
        return [
          ...
          {
            id: 'collaborate',
            title: t('collaborateTitle', { productName }),
            text: (
        ...
        return [
          {
            ...
            title: t('startCollaborating', { productName }),
            text: t('startCollaboratingDescription', { productName }),
            actions: (
              <>
                <Button
                  href="https://catrotics.com"
...
export default function Start(): React.JSX.Element {
  ...
  const startItems = useMemo(() => {
    return [
      ...
      },
      {
        key: 'open-url',
        text: t('openUrl'),
        secondaryText: t('openUrlDescription', { productName }),
        icon: (
          <SvgIcon fontSize="large" color="primary" viewBox="0 0 2048 2048">
  ...
  }, [analytics, dialogActions.dataSource, productName, t])
