...
import { useTopicPublishFrequencies } from '@catrobotics/studio-base/hooks/useTopicPublishFrequences'
  ...
  const updateStats = useCallback(() => {
    ...
    rootRef.current.parentElement?.querySelectorAll('[data-topic]').forEach((field) => {
      ...
      if (field.dataset.topicStat === 'count') {
        if (stat) {
          field.textContent = stat.numMessages.toLocaleString()
        }
        else {
      ...
      if (field.dataset.topicStat === 'frequency') {
        ...
        field.textContent = frequency != undefined ? `${frequency.toFixed(2)} Hz` : EM_DASH
      }
    })
