...
import type {
  FramePromise,
} from '@catrobotics/studio-base/components/MessagePipeline/pauseFrameForPromise'
...
import { Condvar } from '@catrobotics/den/async'
import {
...
export interface MockMessagePipelineProps {
  name?: string
  ...
  setSubscriptions?: (
    arg0: string,
    arg1: Immutable<SubscribePayload[]>,
  ) => void
...
function getPublicState(
  ...
  if (!currentTime) {
    for (const message of props.messages ?? []) {
      if (
        startTime == undefined
        || isLessThan(message.receiveTime, startTime)
      ) {
  ...
  return {
    playerState: {
      ...
      capabilities:
        props.capabilities ?? prevState?.public.playerState.capabilities ?? [],
    ...
    setPlaybackSpeed:
      props.capabilities?.includes(PlayerCapabilities.setSpeed) === true
        ? noop
        : undefined,
...
export default function MockMessagePipelineProvider(
  ...
  if (!currentTime) {
    for (const message of props.messages ?? []) {
      if (
        ...
        || isLessThan(message.receiveTime, startTime.current)
      ) {
            ...
            return {
              ...
              public: {
                ...
                messageEventsBySubscriberId:
                  newState.public.messageEventsBySubscriberId,
              },
            }
          })
        }
      ...
      const reset = () => {
        ...
        set((state) => {
          ...
          return {
            ...
            lastCapabilities: [...publicState.playerState.capabilities],
            public: {
              ...publicState,
  ...
  return (
    <ContextInternal.Provider value={store}>
      {props.children}
