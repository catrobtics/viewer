...
import type { MessageEvent, PlayerState } from '@catrobotics/studio-base/players/types'
...
function lastSeekTime() {
  lastSeekTimeCounter += 1
  return lastSeekTimeCounter
}

function playerStateWithMessages(messages: MessageEvent[]): PlayerState {
  return {
...
function message(headerStampSeconds: number, receiveTimeSeconds: number): MessageEvent {
  return {
    topic: '/foo',
    receiveTime: { sec: receiveTimeSeconds, nsec: 1 },
    message: {
      header: { stamp: { sec: headerStampSeconds, nsec: 1 } },
    },
