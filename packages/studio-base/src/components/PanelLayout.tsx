...
import type { MosaicDropResult, PanelConfig } from '@catrobotics/studio-base/types/panels'
...
import { getPanelIdForType, getPanelTypeFromId } from '@catrobotics/studio-base/util/layout'
...
function TabMosaicWrapper({ tabId, children }: PropsWithChildren<{ tabId?: string }>) {
  ...
  return (
    <div
      className={cx(classes.hideTopLevelDropTargets, 'mosaic-tile')}
      ref={(element) => {
        drop(element)
      }}
    >
...
function selectedLayoutExistsSelector(state: LayoutState) {
  return state.selectedLayout?.data != undefined
}
