...
import Tab from '@catrobotics/studio-base/panels/Tab'
...
export const Basic: StoryObj<{ initialLayoutState: Partial<LayoutData> }> = {
  ...
  render: (args) => {
    ...
    const providers = [
      <PanelSetup key="panel-setup" fixture={fixture}>{undefined}</PanelSetup>,
      <EventsProvider key="events" />,
      <PanelCatalogContext.Provider key="panel-catalog" value={new MockPanelCatalog()} />,
      <MockCurrentLayoutProvider key="current-layout" initialState={args.initialLayoutState} />,
    ]
    return (
