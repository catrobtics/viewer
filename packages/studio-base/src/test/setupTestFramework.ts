...
} from '@catrobotics/studio-base/test/MockSendNotification'
import { vi } from 'vitest'

// Mock out sendNotification for all tests
vi.mock('@catrobotics/studio-base/util/sendNotification', () => {
  return {
    __esModule: true,
...
// If sendNotification was called during a test the test must explicitly clear the expected call.
...
afterEach(async () => {
  if (mockSendNotification.mock.calls.length > 0) {
    ...
    mockSetNotificationHandler()
    throw new Error(
      `sendNotification has been called during this test (call expectSendNotificationCalledDuringTest(); at the end of your test if you expect this): ${JSON.stringify(
        calls,
      )}`,
    )
  }
