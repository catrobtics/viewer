...
import type { MapPanelMessage, Point } from './types'
...
import FilteredPointLayer, {
  POINT_MARKER_RADIUS,
} from '@catrobotics/studio-base/panels/Map/FilteredPointLayer'
import ThemeProvider from '@catrobotics/studio-base/theme/ThemeProvider'
import { darkColor, lightColor, lineColors } from '@catrobotics/studio-base/util/plotColors'
  ...
  const settingsActionHandler = useCallback((action: SettingsTreeAction) => {
    ...
    if (path[1] === 'maxNativeZoom' && input === 'select') {
      setConfig((oldConfig) => {
        ...
        return { ...oldConfig, maxNativeZoom: Number.isFinite(zoom) ? zoom : oldConfig.maxNativeZoom }
      })
    }
