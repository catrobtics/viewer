...
import type { SelectionObject, TabType } from './Interactions'
import type { PickedRenderable } from './Picker'
import type { PublishClickType } from './renderables/PublishClickTool'
import type { InterfaceMode } from './types'
import { useLongPress } from '@catrobotics/hooks'
import {
  PanelContextMenu,
} from '@catrobotics/studio-base/components/PanelContextMenu'

import PublishGoalIcon from '@catrobotics/studio-base/components/PublishGoalIcon'
import PublishPointIcon from '@catrobotics/studio-base/components/PublishPointIcon'
import PublishPoseEstimateIcon from '@catrobotics/studio-base/components/PublishPoseEstimateIcon'
import { usePanelMousePresence } from '@catrobotics/studio-base/hooks/usePanelMousePresence'
import { HUD } from '@catrobotics/studio-base/panels/ThreeDeeRender/HUD'
...
function getSelectionRenderable(selection: MouseEventObject): Renderable | undefined {
  const interactionData = Reflect.get(selection.object, 'interactionData')
  if (interactionData == undefined || typeof interactionData !== 'object') {
    return undefined
  }
  const renderable = Reflect.get(interactionData, 'renderable')
  return renderable instanceof Renderable ? renderable : undefined
}

const useStyles = makeStyles({ name: 'RendererOverlay' })(theme => ({
  root: {
    position: 'absolute',
    top: 10,
    right: 10,
...
export function RendererOverlay(props: Props): React.JSX.Element {
  ...
  const publickClickButtonRef = useRef<HTMLButtonElement>(null)
  ...
  const mousePresenceRef = useRef<HTMLDivElement>(null)
      ...
      {clickedObjects.length > 1 && !selectedObject && (
        <InteractionContextMenu
          ...
          selectObject={(selection) => {
            if (selection) {
              const renderable = getSelectionRenderable(selection)
