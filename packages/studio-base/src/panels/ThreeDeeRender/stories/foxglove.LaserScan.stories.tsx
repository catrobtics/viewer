...
import type { FrameTransform, LaserScan, PointCloud } from '@foxglove/schemas'
...
function HistoryPickingStory(): React.JSX.Element {
  ...
  const laserScan1: MessageEvent<LaserScan> = {
    ...
    message: {
      ...
      start_angle: 0,
      end_angle: Math.PI / 4,
      ranges: Array.from({ length: 10 }).fill(0).map((_, i) => 2 + 0.05 * i),
      intensities: Array.from({ length: 10 }).fill(0).map((_, i) => i),
    },
    schemaName: 'foxglove.LaserScan',
    sizeInBytes: 0,
  }
  ...
  const laserScan2: MessageEvent<LaserScan> = {
    ...
    message: {
      ...
      start_angle: 0,
      end_angle: Math.PI / 2,
      ranges: Array.from({ length: 10 }).fill(0).map((_, i) => 3 + 0.1 * i),
      intensities: Array.from({ length: 10 }).fill(0).map((_, i) => i),
    },
    schemaName: 'foxglove.LaserScan',
    sizeInBytes: 0,
