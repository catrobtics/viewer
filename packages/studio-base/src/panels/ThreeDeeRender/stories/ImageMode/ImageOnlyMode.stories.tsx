...
import { xyzrpyToPose } from '@catrobotics/studio-base/panels/ThreeDeeRender/transforms'
...
import { makeColor, QUAT_IDENTITY } from '../common'
...
function ImageWith3D(initialConfig: ImageModeConfig): React.JSX.Element {
  const topics: Topic[] = [
            ...
            (type, typeIndex) => [
              {
                ...
                points: Array.from({ length: 10 }).fill(0).map((_, i, { length }) => ({
                  x: (0.8 * i) / (length - 1),
              ...
              },
              {
                ...
                color: makeColor('#7995fb', 0.8),
                colors: Array.from({ length: 10 }).fill(0).map((_, i, { length }) => {
              ...
              },
              {
                ...
                scale_invariant: true,
                points: rearrange(
                  Array.from({ length: 10 }).fill(0).map((_, i, { length }) => ({
                    x: (0.8 * i) / (length - 1),
                    y: 0.25 * Math.sin((2 * Math.PI * i) / (length - 1)),
                    z: 0,
                  })),
                ...
                indices: rearrange(Array.from({ length: 10 }).fill(0).map((_, i) => i)),
              },
              {
                // empty points
                type,
