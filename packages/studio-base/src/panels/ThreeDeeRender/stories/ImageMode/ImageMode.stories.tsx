...
import type { CameraInfo, CompressedImage as RosCompressedImage, Image as RosRawImage } from '../../ros'

import { useAsync } from '@catrobotics/hooks'
import Stack from '@catrobotics/studio-base/components/Stack'
import {
  makeCompressedImageAndCalibration,
  makeRawImageAndCalibration,
} from '@catrobotics/studio-base/panels/ThreeDeeRender/stories/ImageMode/imageCommon'
...
import { fireEvent, screen, userEvent, waitFor } from 'storybook/test'
...
function ImageModeRosImage({ imageType }: { imageType: 'raw' | 'png' }): React.JSX.Element {
  const topics: Topic[] = [
  ...
  const cam1: MessageEvent<Partial<CameraInfo>> = {
    ...
    message: {
      ...
      P: [
        ...
        381.22076416015625,
        233.90321350097656,
        -0.00011014656047336757,
        0,
        0,
        1,
        0.000024338871298823506,
      ],
    },
...
function ImageModeFoxgloveImage({
  imageType = 'raw',
  rotation,
  onDownloadImage,
...
function InvalidPinholeCamera(): React.JSX.Element {
  const width = 60
  const height = 45
