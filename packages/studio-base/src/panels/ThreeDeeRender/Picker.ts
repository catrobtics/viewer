...
const FULL_GEOMETRY_GROUP: THREE.GeometryGroup = {
  start: 0,
  count: Infinity,
  materialIndex: 0,
}

function toGeometryGroup(group: unknown): THREE.GeometryGroup | undefined {
  if (group == undefined || typeof group !== 'object') {
    return undefined
  }
  const start = Reflect.get(group, 'start')
  const count = Reflect.get(group, 'count')
  const materialIndex = Reflect.get(group, 'materialIndex')
  return typeof start === 'number' && typeof count === 'number' && typeof materialIndex === 'number'
    ? { start, count, materialIndex }
    : undefined
}

export interface PickedRenderable {
...
export class Picker {
  ...
  #prepareGLRendererForPick(): {
    originalRenderTarget: THREE.WebGLRenderTarget | null
  ...
  #renderItemForPicking = (renderItem: THREE.RenderItem): void => {
    ...
    const group = toGeometryGroup(renderItem.group)
