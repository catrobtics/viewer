...
} from '@catrobotics/studio-base/panels/ThreeDeeRender/SceneExtensionConfig'
...
interface Shared3DPanelState {
  cameraState: CameraState
  followMode: FollowMode
  followTf: undefined | string
}

function isShared3DPanelState(value: unknown): value is Shared3DPanelState {
  if (typeof value !== 'object' || value == undefined) {
    return false
  }
  if (!('cameraState' in value) || typeof value.cameraState !== 'object' || value.cameraState == undefined) {
    return false
  }
  if (
    !('followMode' in value)
    || (value.followMode !== 'follow-pose'
      && value.followMode !== 'follow-position'
      && value.followMode !== 'follow-none')
  ) {
    return false
  }
  return 'followTf' in value && (value.followTf == undefined || typeof value.followTf === 'string')
}
  ...
  useLayoutEffect(() => {
    context.onRender = (renderState: Immutable<RenderState>, done) => {
      ReactDOM.unstable_batchedUpdates(() => {
        ...
        setSharedPanelState(
          isShared3DPanelState(renderState.sharedPanelState)
            ? renderState.sharedPanelState
            : undefined,
        )
