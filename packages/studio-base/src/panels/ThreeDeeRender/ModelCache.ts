...
export class ModelCache {
  ...
  async #loadCollada(
    ...
    manager.itemEnd(url)
    if (dae == undefined) {
      throw new Error(`Failed to parse COLLADA model from ${url}`)
    }
  ...
  #getDracoLoader(manager: THREE.LoadingManager): DRACOLoader {
    ...
    if (!dracoLoader) {
      dracoLoader = new DRACOLoader(manager).setDecoderPath({
...
// The x-catrobotics-converted-tiff protocol is used because the electron protocol handler for
...
function rewriteUrl(url: string): string {
  if (url.startsWith('package://') && /\.tiff?$/i.test(url)) {
    return url.replace('package://', 'x-catrobotics-converted-tiff://')
  }
  return url
}

function unrewriteUrl(url: string): string {
  if (url.startsWith('x-catrobotics-converted-tiff://')) {
    return url.replace('x-catrobotics-converted-tiff://', 'package://')
  }
  return url
}
