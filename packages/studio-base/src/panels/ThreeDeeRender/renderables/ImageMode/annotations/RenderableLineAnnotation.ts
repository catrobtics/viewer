...
class PickingMaterial extends LineMaterialWithAlphaVertex {
  ...
  public override onBeforeCompile(shader: THREE.WebGLProgramParametersWithUniforms, renderer: THREE.WebGLRenderer): void {
...
function makePointsAnnotationFromCircle(
  ...
  return {
    ...
    stamp: circle.stamp,
    style: 'polygon',
    points: Array.from({ length: segments }).fill(undefined).map((_, index) => {
