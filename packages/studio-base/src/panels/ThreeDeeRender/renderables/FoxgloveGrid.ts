...
import type { SettingsTreeAction } from '@catrobotics/studio'

import type { RosValue } from '@catrobotics/studio-base/players/types'
import type { Grid, PackedElementField } from '@foxglove/schemas'
...
import type { SettingsTreeEntry, SettingsTreeNodeWithActionHandler } from '../SettingsManager'
import type {
  ColorModeSettings,
} from './colorMode'
import type { FieldReader } from './pointClouds/fieldReaders'
import { GRID_DATATYPES } from '@catrobotics/studio-base/panels/ThreeDeeRender/foxglove'
  ...
  public updateTexture(foxgloveGrid: Grid, settings: LayerSettingsFoxgloveGrid): void {
    ...
    if (floatMode) {
      ...
      if (!(valueData instanceof Float32Array)) {
        throw new TypeError('Float grid texture has invalid pixel data')
      }
      ...
      if (rgba == undefined) {
        throw new Error('Grid texture has no pixel data')
      }
...
function createTexture(foxgloveGrid: Grid, settings: GridColorModeSettings): THREE.DataTexture {
  ...
  const rows = foxgloveGrid.data.byteLength / row_stride
  const size = Number.isFinite(cols * rows) ? cols * rows : 0
...
const NumericTypeMinMaxValueMap: Record<NumericType, [number, number]> = {
  ...
  [NumericType.UINT32]: [0, 2 ** 32 - 1],
  [NumericType.INT8]: [-128, 127],
  [NumericType.INT16]: [-(2 ** (16 - 1)), -(2 ** (16 - 1)) - 1],
  [NumericType.INT32]: [-(2 ** (32 - 1)), -(2 ** (32 - 1)) - 1],
