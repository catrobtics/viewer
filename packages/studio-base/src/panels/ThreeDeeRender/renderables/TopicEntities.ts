...
import type { BaseUserData } from '../Renderable'
import type { LayerSettingsEntity } from '../settings'
import type { PrimitivePool } from './primitives/PrimitivePool'
import type { RenderableArrows } from './primitives/RenderableArrows'
import type { RenderableCubes } from './primitives/RenderableCubes'
import type { RenderableCylinders } from './primitives/RenderableCylinders'
import type { RenderableLines } from './primitives/RenderableLines'
import type { RenderableModels } from './primitives/RenderableModels'
import type { RenderableSpheres } from './primitives/RenderableSpheres'
import type { RenderableTexts } from './primitives/RenderableTexts'
import type { RenderableTriangles } from './primitives/RenderableTriangles'
    ...
    for (const renderables of this.#renderablesById.values()) {
      for (const renderable of Object.values(renderables)) {
        ...
        if (!updated) {
          ...
          this.renderer.settings.errors.removeFromTopic(
            topic,
            MISSING_TRANSFORM,
          )
        }
    ...
    for (const primitiveType of ALL_PRIMITIVE_TYPES) {
      ...
      if (hasPrimitives) {
        if (!renderable) {
          ...
          Object.assign(renderables, { [primitiveType]: renderable })
          this.add(renderable)
        }
        renderable.update(
          this.userData.topic,
