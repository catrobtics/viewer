...
import type { MessageEvent } from '@catrobotics/studio-base/players/types'

import type { DiagnosticInfo, DiagnosticStatusArrayMsg } from './util'
import { addMessages } from './useDiagnostics'
import { computeDiagnosticInfo, LEVELS } from './util'

function buildMessageAtLevel(level: number): MessageEvent<DiagnosticStatusArrayMsg> {
  return {
    message: {
      status: [
        {
...
function diagnosticInfoAtLevel(level: number): DiagnosticInfo {
