...
import type { OpenSiblingPanel } from '@catrobotics/studio-base/types/panels'
import type { ReactElement } from 'react'
import type { DiagnosticInfo, DiagnosticStatusMessage, KeyValue } from './util'
import Stack from '@catrobotics/studio-base/components/Stack'
import { openSiblingPlotPanel } from '@catrobotics/studio-base/panels/Plot/openSiblingPlotPanel'
import { openSiblingStateTransitionsPanel } from '@catrobotics/studio-base/panels/StateTransitions/openSiblingStateTransitionsPanel'
...
const useStyles = makeStyles({ name: 'DiagnosticStatus' })(theme => ({
  resizeHandle: {
    'position': 'absolute',
    'top': 0,
    'bottom': 0,
...
function hasAnyHtml(value: string): boolean {
  return allowedTags.some(tag => value.includes(`<${tag}`))
}
...
const getFormattedKeyValues = createSelector(
  ...
  (message: DiagnosticStatusMessage): FormattedKeyValue[] => {
    return message.values.map(({ key, value }: KeyValue) => {
      return {
        key,
        keyHtml: hasAnyHtml(key) ? sanitize(key) : undefined,
        value,
        valueHtml: hasAnyHtml(value) ? sanitize(value) : undefined,
      }
    })
  },
)
...
export default function DiagnosticStatus(props: Props): React.JSX.Element {
  ...
  const renderKeyValueCell = useCallback(
    (
      ...
      if (numericPrecision != undefined && isFloatOrInteger(str)) {
        strToRender = Number.parseFloat(str).toFixed(numericPrecision)
      }
  ...
  const renderKeyValueSections = useCallback((): React.ReactNode => {
    ...
    return formattedKeyVals.map(({ key, value, keyHtml, valueHtml }, idx) => {
      ...
      let openPlotPanelIconElem
      if (value.length > 0) {
        openPlotPanelIconElem = !Number.isNaN(Number(value)) ? (
          <IconButton
                ...
                title={(
                  <>
                    <Typography variant="body2">
                      Hardware ID:
                      {' '}
                      <code>{info.status.hardware_id}</code>
                    </Typography>
                    <Typography variant="body2">
                      Name:
                      {' '}
                      <code>{info.status.name}</code>
                    </Typography>
                  </>
                )}
              >
