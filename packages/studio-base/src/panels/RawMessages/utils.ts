...
import type { DiffObject } from '@catrobotics/studio-base/panels/RawMessages/getDiff'

import type { NodeExpansion } from './types'
import { diffLabels } from '@catrobotics/studio-base/panels/RawMessages/getDiff'
...
export function generateDeepKeyPaths(obj: unknown, maxArrayLength: number): Set<string> {
  ...
  const recurseMapKeys = (path: string[], nestedObj: unknown) => {
    ...
    for (const key of Object.getOwnPropertyNames(nestedObj)) {
      ...
      const value = Reflect.get(nestedObj, key)
...
const schemaDocsLinksByDatatype = new Map<string, string>()
for (const schema of Object.values(foxgloveMessageSchemas)) {
  const url = 'https://catrotics.com'
  schemaDocsLinksByDatatype.set(`foxglove_msgs/${schema.name}`, url)
  schemaDocsLinksByDatatype.set(`foxglove_msgs/msg/${schema.name}`, url)
  schemaDocsLinksByDatatype.set(`foxglove.${schema.name}`, url)
}
...
export function getMessageDocumentationLink(datatype: string): string | undefined {
  ...
  const schemaDocsLink = schemaDocsLinksByDatatype.get(datatype)
  if (schemaDocsLink != undefined) {
    return schemaDocsLink
  }
