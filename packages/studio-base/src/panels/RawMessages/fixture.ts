    ...
    '/msgs/big_topic': [
      {
        ...
        message: {
          ...
          timestamp_array: Array.from({ length: 6 })
            .fill(undefined)
    ...
    '/baz/text': [
      {
        ...
        receiveTime: { sec: 123, nsec: 456789012 },
        message: {
          value: Array.from({ length: 10 }).fill('string').join(' '),
          value_long: Array.from({ length: 1024 }).fill('string').join(' '),
          value_with_newlines: Array.from({ length: 1024 })
            .fill(0)
