...
import type { MessagePathStructureItem, PrimitiveType } from '@catrobotics/message-path'
...
function isObjectElement(value: unknown, pathItem: string | number, structureItem: MessagePathStructureItem | undefined): boolean {
  return (
...
function isArrayElement(value: unknown, pathItem: string | number, structureItem: MessagePathStructureItem | undefined): boolean {
  return typeof pathItem === 'number'
...
export function getStructureItemForPath(rootStructureItem: MessagePathStructureItem | undefined, keyPath: (number | string)[]): MessagePathStructureItem | undefined {
  let structureItem: MessagePathStructureItem | undefined = rootStructureItem
  // Walk down the keyPath, while updating `value` and `structureItem`
  for (const pathItem of keyPath) {
