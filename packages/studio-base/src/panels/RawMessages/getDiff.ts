import { isTypicalFilterName } from '@catrobotics/studio-base/components/MessagePathSyntax/isTypicalFilterName'
...
} as const satisfies Record<string, DiffLabel>

export const diffLabelsByLabelText: Record<string, DiffLabel> = _.keyBy(
  Object.values(diffLabels),
  'labelText',
)
...
export default function getDiff({
  ...
  if (Array.isArray(before) && Array.isArray(after)) {
    ...
    if (typeof allItems[0] === 'object' && allItems[0] != undefined) {
      let candidateIdsToCompareWith: Record<
        string,
      ...
      for (const [idKey, candidates] of Object.entries(
        candidateIdsToCompareWith,
      )) {
      ...
      for (const [
        idKey,
        { before: candidateIdBefore, after: candidateIdAfter },
      ] of Object.entries(candidateIdsToCompareWith)) {
        if (
    ...
    if (idToCompareWith != undefined) {
      ...
      for (const beforeItem of before) {
        if (beforeItem == undefined || typeof beforeItem !== 'object') {
          throw new Error(
            'beforeItem is invalid; should have checked this earlier',
          )
        }
        ...
        if (!_.isEmpty(innerDiff)) {
          const isDeleted
            = Object.keys(innerDiff as DiffObject).length === 1
              && Object.keys(innerDiff as DiffObject)[0]
  ...
  if (before == undefined) {
    ...
    const idLabelObj = {
      [diffLabels.ID.labelText]: {
        [idLabel]: { ...(after as DiffObject) }[idLabel],
      },
    }
    return {
  ...
  if (after == undefined) {
    ...
    const idLabelObj = {
      [diffLabels.ID.labelText]: {
        [idLabel]: { ...(before as DiffObject) }[idLabel],
      },
    }
    return {
      [diffLabels.DELETED.labelText]: {
        ...idLabelObj,
        ...(before as DiffObject),
      },
    }
  }

  const beforeText
    = typeof before === 'bigint' ? before.toString() : JSON.stringify(before)
  const afterText
    = typeof after === 'bigint' ? after.toString() : JSON.stringify(after)
