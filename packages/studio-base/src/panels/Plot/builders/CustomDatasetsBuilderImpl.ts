...
import type { Datum, OriginalValue } from '../datum'
import type {
  CsvDataset,
...
import { downsampleScatter } from '@catrobotics/studio-base/components/TimeBasedChart/downsample'
import { extendBounds1D } from '@catrobotics/studio-base/types/Bounds'
...
interface ResetSeriesFullAction {
  type: 'reset-full'
  series: SeriesConfigKey
}

interface ResetSeriesCurrentAction {
  type: 'reset-current'
  series: SeriesConfigKey
}

interface ResetCurrentXAction {
  type: 'reset-current-x'
}

interface ResetFullXAction {
  type: 'reset-full-x'
}

interface UpdateCurrentXAction {
  type: 'append-current-x'
  items: ValueItem[]
}

interface UpdateFullXAction {
  type: 'append-full-x'
  items: ValueItem[]
}

interface UpdateSeriesCurrentAction {
  type: 'append-current'
  series: SeriesConfigKey
  items: ValueItem[]
}

interface UpdateSeriesFullAction {
  type: 'append-full'
  series: SeriesConfigKey
  items: ValueItem[]
}

interface UpdateSeriesConfigAction {
