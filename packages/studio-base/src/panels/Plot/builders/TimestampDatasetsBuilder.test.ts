...
import type { SeriesConfigKey, SeriesItem } from './IDatasetsBuilder'
import { unwrap } from '@catrobotics/den/monads'
import { makeComlinkWorkerMock } from '@catrobotics/den/testing'
...
describe('timestampDatasetsBuilder', () => {
  ...
  it('should cull current messages after threshold is reached', async () => {
    ...
    const messages = Array.from({ length: 60_000 }).fill(1).map((_val, idx) => {
      return {
        topic: '/foo',
        schemaName: 'foo',
