...
} from '@catrobotics/studio-base/components/PanelContextMenu'
import type {
  TimeBasedChartTooltipData,
} from '@catrobotics/studio-base/components/TimeBasedChart/TimeBasedChartTooltipContent'
import type { Bounds1D } from '@catrobotics/studio-base/components/TimeBasedChart/types'
import type {
  TimelineInteractionStateStore,
} from '@catrobotics/studio-base/context/TimelineInteractionStateContext'
import type { SubscribePayload } from '@catrobotics/studio-base/players/types'

import type { SaveConfig } from '@catrobotics/studio-base/types/panels'
import type { PlotConfig } from './config'
import { debouncePromise } from '@catrobotics/den/async'
...
import { fillInGlobalVariablesInPath } from '@catrobotics/studio-base/components/MessagePathSyntax/useCachedGetMessagePathDataItems'
import {
  useMessagePipeline,
...
} from '@catrobotics/studio-base/components/PanelToolbar'
import Stack from '@catrobotics/studio-base/components/Stack'
import TimeBasedChartTooltipContent from '@catrobotics/studio-base/components/TimeBasedChart/TimeBasedChartTooltipContent'
import {
  useClearHoverValue,
  useSetHoverValue,
...
import { VerticalBars } from '@catrobotics/studio-base/panels/Plot/VerticalBars'
import { PANEL_TITLE_CONFIG_KEY } from '@catrobotics/studio-base/util/layout'
import { getLineColor } from '@catrobotics/studio-base/util/plotColors'
import { add as addTimes, fromSec, isTime, toSec } from '@foxglove/rostime'
import { Button, buttonClasses, Fade, Tooltip, useTheme } from '@mui/material'
...
const useStyles = makeStyles({ name: 'Plot' })(theme => ({
  tooltip: {
    maxWidth: 'none',
  },
  resetZoomButton: {
...
export function Plot(props: Props): React.JSX.Element {
  ...
  const hoveredValuesBySeriesIndex = useMemo(() => {
    ...
    const values = Array.from({ length: config.paths.length }).fill('') as string[]
    for (const item of activeTooltip.data) {
      values[item.configIndex] = item.value == undefined ? '' : String(item.value)
    }
