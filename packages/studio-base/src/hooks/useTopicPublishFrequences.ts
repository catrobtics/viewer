...
import type { TopicStats } from '@catrobotics/studio-base/players/types'
import type { Time } from '@foxglove/rostime'
import {
  useMessagePipeline,
} from '@catrobotics/studio-base/components/MessagePipeline'
import { PlayerCapabilities } from '@catrobotics/studio-base/players/types'
import { areEqual, fromMillis, subtract as subtractTimes, toSec } from '@foxglove/rostime'
...
function selectCurrentTime({ playerState }: MessagePipelineContext) {
  return playerState.activeData?.currentTime
}
function selectStartTime({ playerState }: MessagePipelineContext) {
  return playerState.activeData?.startTime
}
const selectEndTime = ({ playerState }: MessagePipelineContext) => playerState.activeData?.endTime
function selectTopicStats(ctx: MessagePipelineContext) {
  return ctx.playerState.activeData?.topicStats ?? EMPTY_TOPIC_STATS
}
