...
import { useStateToURLSynchronization } from '@catrobotics/studio-base/hooks/useStateToURLSynchronization'

import EventsProvider from '@catrobotics/studio-base/providers/EventsProvider'
import { renderHook } from '@testing-library/react'
import { vi } from 'vitest'

vi.mock('@catrobotics/studio-base/context/CurrentLayoutContext')
vi.mock('@catrobotics/studio-base/components/MessagePipeline')

describe('useStateToURLSynchronization', () => {
  it('updates the url with a stable source & player state', () => {
    const spy = vi.spyOn(window.history, 'replaceState');

    (useMessagePipeline as ReturnType<typeof vi.fn>).mockImplementation(selector =>
      selector({
        playerState: {
          activeData: {
