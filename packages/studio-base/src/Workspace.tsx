...
} from '@catrobotics/studio-base/components/DataSourceDialog'
import type {
  MessagePipelineContext,
} from '@catrobotics/studio-base/components/MessagePipeline'
import type { SidebarItem } from '@catrobotics/studio-base/components/Sidebars'

import type { EventsStore } from '@catrobotics/studio-base/context/EventsContext'
import type {
  LeftSidebarItemKey,
  RightSidebarItemKey,
...
import { AppBar } from '@catrobotics/studio-base/components/AppBar'
import {
  DataSourceDialog,
} from '@catrobotics/studio-base/components/DataSourceDialog'
import DocumentDropListener from '@catrobotics/studio-base/components/DocumentDropListener'
import { EventsList } from '@catrobotics/studio-base/components/EventsList'
import KeyListener from '@catrobotics/studio-base/components/KeyListener'
import {
  useMessagePipeline,
  useMessagePipelineGetter,
} from '@catrobotics/studio-base/components/MessagePipeline'
import PanelLayout from '@catrobotics/studio-base/components/PanelLayout'
import PanelSettings from '@catrobotics/studio-base/components/PanelSettings'
import PlaybackControls from '@catrobotics/studio-base/components/PlaybackControls'
import { ProblemsList } from '@catrobotics/studio-base/components/ProblemsList'
import RemountOnValueChange from '@catrobotics/studio-base/components/RemountOnValueChange'
import { Sidebars } from '@catrobotics/studio-base/components/Sidebars'
import Stack from '@catrobotics/studio-base/components/Stack'
import { StudioLogsSettings } from '@catrobotics/studio-base/components/StudioLogsSettings'
import { SyncAdapters } from '@catrobotics/studio-base/components/SyncAdapters'
import { TopicList } from '@catrobotics/studio-base/components/TopicList'
import VariablesList from '@catrobotics/studio-base/components/VariablesList'
import { WorkspaceDialogs } from '@catrobotics/studio-base/components/WorkspaceDialogs'
import { useAppContext } from '@catrobotics/studio-base/context/AppContext'
import { useCurrentUser } from '@catrobotics/studio-base/context/BaseUserContext'
...
import { useDefaultWebLaunchPreference } from '@catrobotics/studio-base/hooks/useDefaultWebLaunchPreference'
import useElectronFilesToOpen from '@catrobotics/studio-base/hooks/useElectronFilesToOpen'
...
import { parseAppURLState } from '@catrobotics/studio-base/util/appURLState'
...
const useStyles = makeStyles({ name: 'Workspace' })({
  container: {
    width: '100%',
    height: '100%',
    display: 'flex',
...
function selectPlayerIsPresent({ playerState }: MessagePipelineContext) {
  return playerState.presence !== PlayerPresence.NOT_PRESENT
}
const selectPlayerProblems = ({ playerState }: MessagePipelineContext) => playerState.problems
function selectIsPlaying(ctx: MessagePipelineContext) {
  return ctx.playerState.activeData?.isPlaying === true
}
...
function WorkspaceContent(props: WorkspaceProps): React.JSX.Element {
  ...
  const allowedDropExtensions = useMemo(() => {
    const extensions = installExtension != undefined ? ['.foxe'] : []
    ...
    return extensions
  }, [availableSources, installExtension])
  ...
  const openHandle = useCallback(
    async (
      ...
      if (file.name.endsWith('.foxe') && installExtension != undefined) {
