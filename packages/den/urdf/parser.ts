...
function parseVec3Attribute(xml: Element, attribName: string): Vector3 | undefined {
  ...
  return { x: Number.parseFloat(x), y: Number.parseFloat(y), z: Number.parseFloat(z) }
}
...
function parseColorAttribute(xml: Element, attribName: string): Color | undefined {
  ...
  return { r: Number.parseFloat(r), g: Number.parseFloat(g), b: Number.parseFloat(b), a: Number.parseFloat(a) }
}
...
function parseFloatContent(xml: Element): number {
  return Number.parseFloat(xml.textContent)
}
