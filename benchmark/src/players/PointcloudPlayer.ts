...
interface FoxglovePose {
  position: { x: number, y: number, z: number }
  orientation: { x: number, y: number, z: number, w: number }
}

interface FoxglovePointCloud {
...
class PointcloudPlayer implements Player {
  ...
  public setParameter(_key: string, _value: unknown): void {
    throw new Error(
      'Parameter editing is not supported by the pointcloud benchmark data source',
    )
  }

  public publish(_request: PublishPayload): void {
    throw new Error(
      'Publishing is not supported by the pointcloud benchmark data source',
    )
  }
  ...
  public async callService(
    ...
    throw new Error(
      'Service calls are not supported by the pointcloud benchmark data source',
    )
  }
