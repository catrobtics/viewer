...
import type { Quaternion } from '@catrobotics/studio-base/util/geometry'
...
class TransformPlayer implements Player {
  ...
  public setParameter(_key: string, _value: unknown): void {
    throw new Error(
      'Parameter editing is not supported by the transform benchmark data source',
    )
  }

  public publish(_request: PublishPayload): void {
    throw new Error(
      'Publishing is not supported by the transform benchmark data source',
    )
  }
  ...
  public async callService(
    ...
    throw new Error(
      'Service calls are not supported by the transform benchmark data source',
    )
  }
  ...
  async #run() {
    ...
    while (!this.#closed) {
      ...
      for (let i = 0; i < TRANSFORMS_PER_TICK; i++) {
        const curTimestamp = rostime.subtract(timestamp, {
          sec: 0,
          nsec: TRANSFORMS_PER_TICK - i,
        })
        messages.push({
          receiveTime: timestamp,
          topic: 'tf',
